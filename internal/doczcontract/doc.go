...
// from IMPL-0005 and R10 from IMPL-0007) and assert the behavior the
...
// DocEntry.Content population; — R6 — the changelog: config block
// (defaults, merge, normalization, enabled-only validation) and
// ParseChangelog's parse shape and ErrNoVersions sentinel; and — R10 —
// the api: config block (dormancy, load-time normalization, enabled-only
// validation behind ErrInvalidAPIPath) and docparse.Title.
//
